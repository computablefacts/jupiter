...
@CheckReturnValue
final public class Scanners implements AutoCloseable {
  ...
  private final List<ScannerBase> batchScannersCache_ = new ArrayList<>();
  private ScannerBase scannerBlob_;
  private ScannerBase scannerIndex_;
  private ScannerBase scannerCache_;
  ...
  public void close() {
    ...
    batchScannersCache_.forEach(ScannerBase::close);

    batchScannersBlob_.clear();
    batchScannersIndex_.clear();
    batchScannersCache_.clear();
  }
  ...
  public ScannerBase cache() {
    if (scannerCache_ == null) {
      scannerCache_ = Tables.scanner(connector_, cacheName(name_), authorizations_);
    }
    return scannerCache_;
  }
   ...
   *         than or equals to 1. A {@link org.apache.accumulo.core.client.BatchScanner} otherwise.
   */
  public ScannerBase cache(int nbQueryThreads) {
    if (nbQueryThreads <= 1) {
      return cache();
    }

    BatchScanner scanner =
        Tables.batchScanner(connector_, cacheName(name_), authorizations_, nbQueryThreads);

    batchScannersCache_.add(scanner);
